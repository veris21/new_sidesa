...
function ganti_password_one(id) {
  ...
  $('#password').show();
  swal({
    title: 'Apa Anda Ingin Merubah Password Anda?',
    text: "Ubah Password Anda",
    type: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
    cancelButtonColor: '#d33',
    confirmButtonText: 'Iya, Ganti Password!'
  }, function isConfirm() {
    $.ajax({
      url: url,
      type: "GET",
      dataType: "JSON",
      success: function (data) {
        setting_method = 'update_password';
        $('#modal_setting_user').modal('show');
        // $('.modal-title').text('Update Data Pengguna');
      },
      error: function (jqXHR, textStatus, errorThrown) {
        swal('Astagapeer', 'Ade Nok Salah Mudel e...!', 'error');
      }
...
function edit_user_one(id) {
  ...
  $('#set_uid').show();
  swal({
    title: 'Apa Anda Ingin Merubah Data Pengguna?',
...
function save_setting_one() {
    ...
    $.ajax({
      url: url,
      type: 'POST',
      data: $('#setting_user').serialize(),
      dataType: 'JSON',
      success: function (data) {
        swal('Selamat !', 'Data Pengguna Berhasil diperbaharui  Ke Sistem!', 'success');
        location.reload();
      },
      error: function (jqXHR, textStatus, errorThrown) {
        swal('Astagapeer', 'Ade Nok Salah Mudel e...!', 'error');
